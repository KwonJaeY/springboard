        ...
        <div class="container"> <!-- container -->
            <div class="section-header">
                ...
                <c:choose>
               		...
               		<c:redirect url = "AcademyGradeManagementList?page=1&fileCode=0100"></c:redirect>
               		</c:when>
                        ...
                        <div class="post-wrap"> <!-- Post Wrapper -->
                            <div class="media post"> <!-- post wrap -->                               
                                <div class="media-left">
                               	 <a href="AcademyGradeManagementView?num=${rList.num }&fileCode=${fileCode}"><h5 class="media-heading"><strong>${Total }. ${rList.title}</h5></a>	    
                                </div>                              
                                <div class="media-body">                                                                   
                                    <p>${rList.content},${rList.grade}등급,석차 : ${rList.ranking}</p>
					...
					<nav>
						<ul class="pager">
						    <c:if test="${Paging.prev }">
							    <li>
							        <a href='<c:url value="/AcademyGradeManagementList?page=${Paging.startPage-1 }&fileCode=${param.fileCode }"/>'><i class="fa fa-chevron-left"></i></a>
							    </li>
							</c:if>
							<c:forEach begin="${Paging.startPage }" end="${Paging.endPage }" var="pageNum">
							    <li>
							        <a href='<c:url value="/AcademyGradeManagementList?page=${pageNum }&fileCode=${param.fileCode }"/>'><i class="fa" id = "nowNum${pageNum}">${pageNum }</i></a>
							    </li>
							</c:forEach>
							<c:if test="${Paging.next && Paging.endPage >0 }">
							    <li>
							        <a href='<c:url value="/AcademyGradeManagementList?page=${Paging.endPage+1 }&fileCode=${param.fileCode }"/>'><i class="fa fa-chevron-right"></i></a>
							    </li>
						    </c:if>						    	
						</ul>
	                </nav>	                
				</article>
				<button class = "btn btn-default pull-right" onclick="location.href = 'AcademyGradeManagementInsert'">글 등록</button>                
            </div><!-- end container -->
