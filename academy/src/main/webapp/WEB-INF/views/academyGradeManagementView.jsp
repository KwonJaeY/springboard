        ...
        <div class="container"> <!-- container -->
            <div class="section-header">
                ...
                <c:choose>
               		...
               		<c:redirect url = "AcademyGradeManagementView?num=${param.num }&page=1&fileCode=0100"></c:redirect>
               		</c:when>
			...
			<article>
				...
				<table class="table">				
					<tbody>
						...
						<tr>
							<th>등급</th>
							<td><input type="text" maxlength="1" name="grade" value="${rlist.grade }" maxlength="255" required="required" class="form-control"></td>
						</tr>
						<tr>
							<th>석차</th>
 			...
 			<article>
	 			<c:choose>
	 				<c:when test="${loginRole eq 'admin'}">
						<button type="submit" class="btn btn-default pull-right" id = "insert1">게시글수정</button>
						<button type="button" class="btn btn-default pull-right" onclick = "location.href = 'AcademyDel?table=grademanagement&condition=num&conditionVal=${rlist.num}'">게시글삭제</button>
	 				</c:when>
	 				<c:when test="${loginId == rlist.writerId}">
						<button type="submit" class="btn btn-default pull-right" id = "insert2">게시글수정</button>
						<button type="button" class="btn btn-default pull-right" onclick = "location.href = 'AcademyDel?table=grademanagement&condition=num&conditionVal=${rlist.num}'">게시글삭제</button>
	 				</c:when>
	 			</c:choose>	
			</article> 
